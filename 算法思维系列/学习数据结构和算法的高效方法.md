...
在遵循二叉树前中后序通用遍历的框架的基础上，需要审题仔细:
- 1.首先一个难点是要确定：递归的本体**是要寻找子树到当前节点的最大和**
- 2.本题表示无需经过根路径，所以第二个难点是要发现所有递归体内的子树也可以是解，所以要在递归逻辑内做判断并记录。

Go语言版本如下:

````go
var ans = -10000
func getOneSideMax(root *TreeNode) {
	if root == nil {
	    return 0	
    }
    leftMax := max(0, getOneSideMax(root.Left))
    rightMax := max(0, getOneSideMax(root.Right))
